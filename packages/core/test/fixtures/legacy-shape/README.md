...
archive currently published under `examples/showcase/public/data/`.

Regenerating it would re-encode the corpus into the new compact shapes and turn
`test/legacy-shape-backcompat.test.ts` into a test of the new path instead of
the old one. That test walks the raw frames and asserts the three keys are
absent, so it would fail loudly rather than silently — but the fix is to
restore these bytes, never to relax the assertion.
...
v5 directory (the technique `helpers/packed-fixture.ts` already uses); the
