...
| `widthScale`                         | `number`                           | `1`                 | Global multiplier for arc widths (`ArcLayer` pass-through).                                                                                                                                                                                                                                                                                      |
| `widthMinPixels`                     | `number`                           | `0`                 | Clamp arc width to at least this many on-screen pixels.                                                                                                                                                                                                                                                                                          |
| `widthMaxPixels`                     | `number`                           | `MAX_SAFE_INTEGER`  | Clamp arc width to at most this many on-screen pixels.                                                                                                                                                                                                                                                                                           |
...
| `colorPalette`                       | `Color[]`                          | 10-stop             | Palette for a categorical `sourceColor`/`targetColor` column, indexed by first-seen category order within each tile.                                                                                                                                                                                                                             |
...
| `filterProperty`                     | `string \| null`                   | `null`              | Name of a baked **numeric** column to GPU-filter arcs by ([`STTDataFilterExtension`](./data-filter-extension.md)); composes WITH the time filter — an arc must pass both. Accessor-alias of deck's `getFilterValue`: a column NAME, not a function. Unset ⇒ the extension is not installed at all. A tile lacking the column renders unfiltered. |
