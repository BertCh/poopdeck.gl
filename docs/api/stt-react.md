...
constructs the `TimeController` + `PlaybackGovernor`, drives a 10 Hz-throttled UI
...
| `initialAutoSpeed` | `boolean`                        | `false`           | Start in Auto-speed mode instead of the fixed 1× preset. Mount-time seed only (like `loop`): any explicit preset exits Auto, `onAutoSpeedSelect` re-enters it, and a later `timeRange` change resets to 1×. |
...
| `currentTime`                                                    | `number`                                       | 10 Hz-throttled UI clock (slider/label only, not the layers).                                                                                                        |
...
| `timeRange`              | `{ start: number; end: number }`    | Optional. Full data range; drives the bar geometry and slider bounds. Omitted, the bar falls back to the degenerate `[currentTime, currentTime]` range, so `<PlaybackControls {...usePlayback({})} />` typechecks.                                                                                                                                                                                                     |
...
| `compact`                | `boolean`                           | Optional (default `false`). Phone-sized layout: one row less (the range-endpoint labels drop), the five speed radios and the fine slider folded behind a single speed chip, and larger touch targets; the hover-only preview toggle and the keyboard-shortcut sheet drop out. A prop, not a media query — only the host knows whether the bar is narrow because of the viewport or because it sits in a 320px sidebar. |
