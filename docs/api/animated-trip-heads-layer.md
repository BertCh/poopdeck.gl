...
Colors each dot by a per-vertex scalar carried on the trip, interpolated to the
dot's live position exactly like the position itself, then mapped through a
ramp. Mirrors [`AnimatedTripsLayer`](./animated-trips-layer.md)'s `gradient*`
vocabulary, so one config drives both the streamline and the dot renderer.

| Property            | Type               | Default  | Description                                                                                                                                                                                                          |
| :------------------ | :----------------- | :------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gradientProperty`  | `string \| null`   | `null`   | Names which per-vertex scalar channel to color by — the one supported channel is `'vertexValues'`. Unset, or a tile without the channel, falls back to the constant `headColor`; so does a `NaN` interpolated value. |
| `gradientDomain`    | `[number, number]` | `[0, 1]` | Value range mapped onto the ramp.                                                                                                                                                                                    |
| `gradientColorRamp` | `Color[]`          | `[]`     | Low→high color stops (piecewise-lerped). Empty ⇒ constant color.                                                                                                                                                     |

### 3D elevation

Every source below is **added** to whatever z the tile geometry already
carries.

| Property                    | Type                                                     | Default | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| :-------------------------- | :------------------------------------------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `elevationProperty`         | `string \| null`                                         | `null`  | Numeric (or categorical, via `elevationMapping`) tile column setting each dot's altitude in metres. Per-**feature**, not per-vertex: a trip rides one level for its whole life (e.g. a wind particle pinned to a pressure surface). Unset ⇒ dots stay at ground.                                                                                                                                                                                               |
| `elevationMapping`          | `Record<string, number> \| null`                         | `null`  | Category-string → metres map for a categorical `elevationProperty` (the height analogue of `gradientColorRamp`).                                                                                                                                                                                                                                                                                                                                               |
| `elevationScale`            | `number`                                                 | `1`     | Multiplier applied to both `elevationProperty` and `elevationFromVertexValues` — the scene's shared vertical exaggeration.                                                                                                                                                                                                                                                                                                                                     |
| `getTerrainElevation`       | `((lon: number, lat: number) => number \| null) \| null` | `null`  | Terrain drape: a per-frame height probe sampled at each dot's live position, so dots ride the ground surface of a 3D basemap. The returned z is used **as-is** (the caller owns exaggeration and any anti-sink lift); `null` means "height unknown here" and leaves z untouched. Runs once per active dot per frame, so it must be O(1) — memoized/bilinear lookups, never network fetches.                                                                    |
| `elevationFromVertexValues` | `boolean`                                                | `false` | Treat the tile's per-vertex `vertexValues` channel as elevation in metres — a DEM baked per vertex at build time (`stt-generate gtfs --bake-elevation`). Interpolated at the dot's live position, scaled by `elevationScale`. Zero per-frame terrain queries, so it is the preferred way to ride a 3D basemap; `getTerrainElevation` is the runtime fallback for archives lacking the channel. Compatible with gradient coloring — both read the same channel. |
...
1. **Tile prepare** (once per tile, cached): the tile's `positions`/`startIndices`/`startTimes`/`endTimes` are referenced zero-copy; per-vertex times come from `vertexTimestamps` when present, otherwise they are synthesized distance-proportionally. The gradient scalar channel and the per-feature elevations are resolved into the same cached record.
