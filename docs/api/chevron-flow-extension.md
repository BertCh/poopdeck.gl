...
| `directionColors`        | `[[r,g,b], [r,g,b], [r,g,b], [r,g,b]]` | amber/green/teal/violet | The four cardinal colors `[N, E, S, W]`, each `[r, g, b]` in 0–255 (RGB only — alpha is not accepted), placed at bearings `offset`, `offset + 90`, `offset + 180`, `offset + 270` and interpolated cyclically for in-between bearings. **Interned by content** at construction, so an inline literal written fresh in every React render still yields the same options reference and does not thrash the shader cache — see [Performance](#performance).                                                                                           |
...
```
chevronS    = along + abs(across) * skew * dir
chevronBand = fract(chevronS / period' - (phase / period) * sign(dir))
chevronHead = 1 - smoothstep(duty - feather, duty, chevronBand)
```

`period'` is the raw `period` unless `uniformSpacing` refits it per segment;
the phase is applied in **turns** (after the divide, against the raw `period`)
so a refitted divisor cannot shift the band at each wrap.
...
**It combines with a marching `speed`.** Because the phase is applied in turns
against the raw `period` (see [The chevron band](#the-chevron-band)), arrows
advance one arrow-spacing per `period / speed` seconds on every segment —
uniform in _arrows_ per second even where the fitted spacing differs from
`period`.
