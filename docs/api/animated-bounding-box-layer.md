...
The `AnimatedBoundingBoxLayer` renders **one smooth-moving oriented 3D box per tracked object** — the streetscape.gl / avs.auto tracked-object look, and the AV-cockpit detected-object overlay. The tile archive carries one POINT feature per object _per keyframe_ (a snapshot: `track_id`, `category`, `heading`, `length`/`width`/`height`, `speed`, timestamped). The layer pools every loaded tile's keyframes by `track_id` and, once per frame, CPU-interpolates each active track's pose between the two keyframes bracketing the playhead — so an object glides continuously instead of leaving a "train" of one box per keyframe behind it.
...
| `wireframe`            | `boolean`                          | `false`                              | Draw a line wireframe around each box instead of filled faces (`SimpleMeshLayer` pass-through). This is the mesh's _triangle_ wireframe (diagonals on every face); for a clean detection-box outline use `stroked` instead.                                                                                                                            |
...
Cost scales with the number of _active_ tracks over the visible tiles (AV scenes carry tens of active objects and a few thousand snapshots per scene) — a binary-search + lerp per active track, well under a millisecond per frame.
