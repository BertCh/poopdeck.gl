...
**2026-08-24:** the 28.5 fps reading **did not reproduce** — 116 fps on
ANGLE/Metal, and `earthquake-activity` reads 117 fps once the loader loops were
closed. See [tile-loading-audit-2026-08.md](./tile-loading-audit-2026-08.md)
§9.3.1 and §9.3.5.
...
**2026-08-24:** both toplines this section rests on have moved — `/drive`'s
28.5 fps did not reproduce (116 fps on ANGLE/Metal) and `earthquake-activity`
reads 117 fps with the loader loops closed
([tile-loading-audit-2026-08.md](./tile-loading-audit-2026-08.md) §9.3.1,
§9.3.5). §5.1's consolidation proposal should be re-priced before it is
scheduled.
...
**Post-split note (2026-08-26).** §8 was captured before the repository split.
The nine `rust*` / `python` rows in §8.3, and the determinism-lane,
ordering-simulator, `stt-optimize` and `stt-validate` rows in §8.1, are now
STT-repo instruments (`stt:.github/workflows/ci.yml`); this repo's `ci.yml`
runs `typescript-lint`, `typescript` and `showcase-probe`. The tables stand as
the dated record of what was read that day.
...
| Cold start                             | YES                              | `pnpm --filter @poopdeck.gl/bench bench:cold-start -- [dataset ...] [--json] [--verbose] [--repeat N] [--base URL] [--cache-bust]`                                                                                               | stdout table or `--json`: requests / bytes to first frame, per-object breakdown, `cf-cache-status`                     | P0-4 (K10), then M4/§7 pricing             |
...
| Golden-pin gate                        | YES (new, P0-6)                  | `node .github/scripts/check-golden-pins.mjs [--base <ref>] [--working-tree]`; self-test `node --test .github/scripts/check-golden-pins.test.mjs`                                                                                 | pass/fail + the problems §13.1 rationale on failure                                                                    | guards M2/R1 (Phase 4)                     |
...
**Golden-pin gate scope, stated precisely.** `PINNED_ROOTS` differs per
repository: the STT copy watches `stt:crates/stt-core/tests/fixtures/v2-golden/**`
plus any file named `expected-hashes.json`, while
[this repo's copy](../../.github/scripts/check-golden-pins.mjs) watches
`packages/core/test/fixtures` only — the `expected-hashes.json` rule is
writer-side and stays upstream (its docblock at `:70-76` says so). As first read
the gate did **not** watch
...
**R1 — RESOLVED 2026-08-26.** The reader now accepts a **range**, not an
equality: `MIN_PACKED_FORMAT_VERSION = 2` and `PACKED_FORMAT_VERSION = 3`
(`packages/core/src/archive.ts:106`, `:119`) with the check at `:2609-2612`
erroring only outside `2..3`; the Rust reader matches
(`stt:crates/stt-core/src/pack/mod.rs:63`, `:75`). The fleet is at
`formatVersion: 3` and `cold-start.mjs` runs on the working tree. The entry
below is the 2026-08-10 record.

**R1 — `cold-start.mjs` cannot run on the working tree.**
`packages/core/src/archive.ts:96` carries an uncommitted bump of
`PACKED_FORMAT_VERSION` from `2` (at HEAD `stt:5bc30e3`) to `3`, and the manifest
...
- **Anything minted now is `formatVersion: 3`.** Both writers in this tree are
  ...
  HEAD `stt:5bc30e3`'s reader is at 2 and gates on strict equality. A fixture minted
...
`git -C <stt-checkout> show 5bc30e3:tools/bench/baselines/earthquakes-ci.stt` —
that SHA is pre-split history and resolves only in the STT repository); the three baseline
...
**The harness could not run as specified.** See §8.4 R2 (three-dataset table);
the R1 version skew recorded alongside it is since resolved — the reader accepts
`2..3`. The capture was run against a scratchpad build of the
...
tools/bench/src/` reports the harness unmodified). HEAD is `stt:5bc30e3`, level
...
⚠️ The `gtfs-ch` row measures the archive **as deployed on 2026-08-10**. It was
rebuilt 2026-08-23 into a much larger build — 2.4 GB on disk, `feature_count`
6,275,668, `tile_count` 557,899 — so the row understates the current archive by
roughly 5× on bytes; see [tile-loading-audit-2026-08.md](./tile-loading-audit-2026-08.md)
F3 / CS-6. Re-measure before quoting it.
...
1. **Single-page directories are fetched whole, as a `200` GET, not a range.**
   ...
   three; the problems §7.2 DP must price the 1-page case explicitly.
...
| Reader     | `@poopdeck.gl/core` built from HEAD `stt:5bc30e3` (`PACKED_FORMAT_VERSION 2`) |
...
> closed all four O1 cells at once**, and the same session closed the problems §11.6
> metrics as far as one route can. Seven open → **two** (O2 sampling error, O5
> pitched bytes) plus a scoped remainder on problems §11.6. The strikethrough rows below
> record what closed them; the method, the caveats and the two harness defects
> the pass uncovered are in **§10.5** (O1) and **§10.6** (problems §11.6). Read those
...
| problems §11.6 scrub metrics         | **PARTLY CLOSED 2026-08-11 (§10.6)** — all five metrics now measured off-state and on-state, but on `/demo/storm-radar`, which is none of the three named heavy routes       | The three heavy routes (NYC taxi, `/drive`, BIXI), which need v3 archives to exist |
...
- **O1** — five rows (horizon feasibility, byte-metered budgets, cadence
  tolerance, decode priority, no frame-cost regression), covering
  problems §9.1–§9.4, §10.2, §11.2 and §11.3.
...
- Plus the **problems §11.6** scrub keep-vs-delete row, which is a recorded decision the
...
### 10.6 The problems §11.6 scrub run (2026-08-11) — the off-state baseline, and the on-state pass
...
in `scrub-cost-storm-radar.json` and it is **not** the problems §11.6 evidence.
...
names** — problems §11.6 slates the wiring for deletion only on failure across NYC taxi,
`/drive` and BIXI together, and at the time all three were unopenable in this
tree (v2 archives; no cheap local `.parquet` for a v3 rebuild). The reader has
since widened to `2..3`, so what remains is the re-run, not a blocker. So
**DT-4 and the problems §11.6
...
DT-4 is not a mechanism; it is a **recorded decision procedure** over the problems §11.6
...
| problems §11.6 criterion                   | Threshold                          | Measured (3 runs)        |              |
| ------------------------------------------ | ---------------------------------- | ------------------------ | ------------ |
...
**Why it is not discharged: one route, and the wrong one.** problems §11.6 names NYC
...
routes resolved at the time to `formatVersion: 2` archives the working-tree
reader then refused (the §8.4 R1 skew, since resolved — the reader accepts
`2..3`; the re-run on the named routes is simply outstanding). A motion tier's
value is a function of how much detail it drops,
...
- **long-lived bytes** — of those, the bytes attributable to tiles the window
  ...
  start bucket. That is the problems §2.2 pathology stated as a quantity;
...
multi-day storm-cell archives — were not re-run against it (the reader-version
blocker recorded as §8.4 R1 is since resolved; the re-run is simply outstanding).
The instrument is the durable deliverable; re-run it on those
datasets and the condition may yet fire. The 25 % threshold itself is
